...
var obj = {
    query: function() {
        $(grid_selector).jqGrid({
            url: config.ordersQueryStuUrl + "?type=OUT_APPLY",
            datatype: "json",
            height: 'auto',
            colNames: ['退学时间', '姓名', '产品', '课程', '来源', '操作'],
            colModel: [
                { name: 'retreatFeeDate', index: 'retreatFeeDate', width: 160, sortable: false, formatter: formatDate },
    ...
    modifyObj: function(id) {
        window.location.href = config.ordersFormYBMUrl + "?type=OUT_APPLY&id=" + id;
    },
    ...
    refreshTable: function() {
        ...
        //$(grid_selector).setGridParam({url:config.ordersQueryUrl+"?type=OUT_APPLY",postData:params}).trigger('reloadGrid');
        $(grid_selector).setGridParam({ url: config.ordersQueryStuUrl + "?type=OUT_APPLY", postData: params }).trigger('reloadGrid');
    }
};
