...
var obj={
	query:function(){
		$(grid_selector).jqGrid({
			url: config.ordersQueryUrl+"?type=PEND_APPLY",
			datatype: "json",
			height:'auto',
			...
			colModel:[
				...
				{name:'',index:'', width:150, fixed:true, sortable:false, resize:false, formatter:actFormatter
				}
			],
			rownumbers: true,
			hidegrid: false,
	...
	modifyObj:function(id){
		window.location.href=config.ordersFormUrl+"?type=PEND_APPLY&id="+id;
	}
	,
	init:function(){
		obj.query();
	...
	refreshTable:function(){
		...
		$(grid_selector).setGridParam({url:config.ordersQueryUrl+"?type=PEND_APPLY",postData:params}).trigger('reloadGrid');
	}
};
